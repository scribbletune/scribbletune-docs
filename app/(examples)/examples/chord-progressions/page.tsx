...
const ChordProgressionsPage = () => (
  <div>
    ...
    <p>Scribbletune can be used to generate chord progressions based on basic music theory.</p>
    <CodeBlock code={`const scribble = require('scribbletune');
console.log(scribble.progression('M')); // outputs an array, e.g. ['I', 'ii', 'iii', 'V']`} />
    <p>We can use Scribbletune’s <code>getChordsByProgression</code> method to generate the chords from this progression:</p>
    <CodeBlock code={`const scribble = require('scribbletune');
...
console.log(scribble.getChordsByProgression('C4 major', majorChordProgression)); // outputs something like 'Am-4 Dm-4 Dm-4 GM-4'`} />
    <p>Taking this one step ahead, we can now use the chords we generate along with Scribbletune’s <code>arp</code> method and patterns with the <code>R</code> character to create nice sounding arpeggios!</p>
    <CodeBlock code={`const scribble = require('scribbletune');
