...
const clip = scribble.clip({
  ...
  return (
    <div>
      ...
      <p className="mb-4">The clip method takes an object literal as an argument. This object lets you define the parameters of that clip. Here is an example of the clip method being called with an object that sets up the most basic properties of the clip:</p>
      ...
      <div className="space-y-4">
        ...
        <PianoRoll notes={['C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'B4']} pattern="x_x_x_x_x_x_x_" />

        <div>
          <h3 className="text-xl font-semibold">pattern</h3>
          <code className="bg-gray-800 rounded text-red-500">{`String`}</code>
          <p className="mt-4">This is the most important parameter for the clip method’s object. It abstracts away the MIDI note on and note off events along with the individual note durations and few other MIDI instructions to form a simple notation language. It is made up only of the following characters:</p>
          ...
          <p className="mb-4">This is native to Scribbletune and it’s used in multiple ways across the Scribbletune library. Here’s an example of what a very simply pattern looks like,</p>
          <CodeBlock code={'xxxx'} />
      
          <p className="mb-4">These are 4 quarter notes. By default a single x is a quarter note. The default can be changed to eighth note or half note or any other standard note duration. It might look like this in a DAW (if exported as MIDI from Scribbletune):</p>
          <PianoRoll notes={['C4']} pattern="x___x___x___x___" />
          <p className="my-4">Each x implies a MIDI "note on" event (hence shows a note in the piano roll when imported) and each hyphen implies a MIDI "note off" event which does not have any note in that location in the clip. Other than setting note on and off events, we can even set the duration of a note on event using the pattern language’s third character: the _ (underscore) character:</p>
          <CodeBlock code={'x_'} />
          <p className="mb-4">This makes the quarter note double in size making it a half note</p>
          <PianoRoll notes={['C4']} pattern="x_______" />
          <p className="my-4">Notes can be divided further by placing them in square braces. Square braces also allow nesting more square braces leading to the creation of interesting or even complex patterns!</p>
          ...
          <p className="my-4">Lastly, the letter R will let you randomly decide if a note is to be added or not at the specified position. For example, in this pattern xxRx, the quarter note at R will be randomly picked to be added or not (random midi note on/off). The volume of this note is governed by accentLow property of the clip.</p>
          <p className="my-4">If you set an additional property called <code className="bg-gray-800 rounded text-red-500">randomNotes</code> with a string or array of notes, R will then randomly select a note to play from these provided notes instead of randomly deciding note on or note off events.</p>
          <p className="my-4"><code className="bg-gray-800 rounded text-red-500">R</code> is a powerful feature to create interestingly intricate patterns.</p>
        </div>
